...
<html lang="es">
    ...
    <body  class="body" bgcolor="#e0766e">
        <form method="GET" action="PrestamoServlet"  class="div_reg">
            <h1 class="login_title">Crear Préstamo</h1>
            <h3 class="login_text">ID</h3>
            <input type="text" name="id_alquiler" class="entrada" value="" placeholder="1706220829" required>

            <h3 class="login_text">Fecha prestamo</h3>
            <input type="date" name="f_salida" class="entrada" value="Fecha de prestamo" required>

            <h3 class="login_text">Fecha Devolucion</h3>
            <input type="date" name="f_entrada" class="entrada" value="Fecha de devolucion" required>

            <h3 class="login_text">Empleado</h3>
            <select type="text" name="ID_empleado" class="entrada" value="" required>
                <option value="" selected disabled>Seleccione el empleado dando el prestamo</option>
            ...
            <select type="text" name="id_libro" class="entrada" value="" required>
                <option value="" selected disabled>Seleccione el libro que se está prestando</option>
                <% for (int i = 0; i < libros.size(); i++) {%>
                <option value="<%=libros.get(i).getId_libro()%>" > <%out.print(libros.get(i).getTitulo());%></option>
                <%}%>
            </select>
            <h3 class="login_text">Cliente</h3>
            <select type="text" name="ID_cliente" class="entrada" value="" required>
                <option value="" selected disabled>Seleccione el cliente al que se le hará el préstamo</option>
